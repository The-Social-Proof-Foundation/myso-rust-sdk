    ...
    impl super::StakingPool {
        pub const fn const_default() -> Self {
            Self {
                ...
                exchange_rates: None,
                pending_stake: None,
                pending_total_myso_withdraw: None,
        ...
        ///If `myso_balance` is set, returns [`Some`] with a mutable reference to the value; otherwise returns [`None`].
        pub fn myso_balance_opt_mut(&mut self) -> Option<&mut u64> {
            self.myso_balance.as_mut().map(|field| field as _)
        }
        ///Returns a mutable reference to `myso_balance`.
        ///If the field is unset, it is first initialized with the default value.
        pub fn myso_balance_mut(&mut self) -> &mut u64 {
            self.myso_balance.get_or_insert_default()
        }
        ///If `myso_balance` is set, returns [`Some`] with the value; otherwise returns [`None`].
        pub fn myso_balance_opt(&self) -> Option<u64> {
            self.myso_balance.as_ref().map(|field| *field)
        }
        ///Sets `myso_balance` with the provided value.
        pub fn set_myso_balance(&mut self, field: u64) {
            self.myso_balance = Some(field);
        }
        ///Sets `myso_balance` with the provided value.
        pub fn with_myso_balance(mut self, field: u64) -> Self {
            self.set_myso_balance(field);
            self
        }
        ...
        ///If `pending_total_myso_withdraw` is set, returns [`Some`] with a mutable reference to the value; otherwise returns [`None`].
        pub fn pending_total_myso_withdraw_opt_mut(&mut self) -> Option<&mut u64> {
            self.pending_total_myso_withdraw.as_mut().map(|field| field as _)
        }
        ///Returns a mutable reference to `pending_total_myso_withdraw`.
        ///If the field is unset, it is first initialized with the default value.
        pub fn pending_total_myso_withdraw_mut(&mut self) -> &mut u64 {
            self.pending_total_myso_withdraw.get_or_insert_default()
        }
        ///If `pending_total_myso_withdraw` is set, returns [`Some`] with the value; otherwise returns [`None`].
        pub fn pending_total_myso_withdraw_opt(&self) -> Option<u64> {
            self.pending_total_myso_withdraw.as_ref().map(|field| *field)
        }
        ///Sets `pending_total_myso_withdraw` with the provided value.
        pub fn set_pending_total_myso_withdraw(&mut self, field: u64) {
            self.pending_total_myso_withdraw = Some(field);
        }
        ///Sets `pending_total_myso_withdraw` with the provided value.
        pub fn with_pending_total_myso_withdraw(mut self, field: u64) -> Self {
            self.set_pending_total_myso_withdraw(field);
            self
        }
