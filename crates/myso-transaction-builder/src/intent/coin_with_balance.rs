...
impl CoinWithBalance {
    ...
    pub fn myso(balance: u64) -> Self {
        Self {
            coin_type: StructTag::myso(),
            balance,
            use_gas_coin: true,
        }
    }
    ...
    // This parameter is only respected when coin type is `MYSO`.
    ...
    async fn resolve(
        ...
        for (id, intent) in builder.intents.extract_if(.., |_id, intent| {
            ...
            if request.balance == 0 {
                ...
                let coin_type = if request.coin_type == StructTag::myso() && request.use_gas_coin {
                    CoinType::Gas
                } else {
...
impl CoinWithBalanceResolver {
    ...
    async fn resolve_coin_type(
        ...
        // For MYSO need to handle working with gas coin
    ...
    async fn resolve_gas_coin(
        ...
        let mut coins = client
            //TODO populate excludes
            .select_coins(&sender, &(StructTag::myso().into()), sum, &[])
            .await?
            .into_iter()
